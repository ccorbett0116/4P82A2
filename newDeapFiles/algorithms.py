...
def eaSimple(populations, toolboxes, cxpbs, mutpbs, ngen, stats=None,
             halloffames=None, verbose=__debug__, elitism=True, k=0):
    ...
    for population, halloffame, toolbox, logbook in zip(populations, halloffames, toolboxes, logbooks):
    ...
    for gen in range(1, ngen + 1):
        for population, halloffame, toolbox, cxpb, mutpb, logbook in zip(populations, halloffames, toolboxes, cxpbs, mutpbs, logbooks):
        ...
        if gen % k ==0 and len(populations) != 1:
            populations = toolbox.migration(populations)

        for population, logbook in zip(populations, logbooks):
            record = stats.compile(population) if stats else {}
